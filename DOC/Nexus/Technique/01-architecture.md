...
- `platform-api` : API, persistance PostgreSQL et orchestration métier.
- `platform-core::nexus` : entités et règles des jeux, wallets et serveurs.
- `platform-scheduler` : déclenche les tâches périodiques via `platform-api`.
- runtime de jeux : exécution des serveurs de jeu, en mode Docker ou noop selon la configuration.
- web : tableau de bord via la passerelle `/platform-api/`.
