...
use nexus_api::adapters::inbound::grpc::NexusGrpcService;
use nexus_api::adapters::inbound::http::{build_router_with, HttpConfig};
use nexus_api::bootstrap::AppState;
use nexus_core::domain::entities::game::server::{
...
use nexus_core::domain::entities::game::template::GameTemplate;
use nexus_core::domain::entities::system::bot_config::{BotDefinition, BotGuildConfig};
use nexus_core::domain::entities::wallet::{Wallet, WalletTransaction};
use nexus_core::domain::entities::wheel::WheelCaseData;
...
use nexus_core::ports::inbound::coussin_profile::{CoussinCombatUseCase, CoussinProfileUseCase};
use nexus_core::ports::inbound::coussin_steal::CoussinStealUseCase;
use nexus_core::ports::inbound::game::manage_game_servers::{
...
use nexus_core::ports::inbound::play_wheel::{PlayWheelCommand, PlayWheelResult, PlayWheelUseCase};
use nexus_core::ports::inbound::transfer_coins::{
...
use nexus_core::ports::inbound::wheel_cases::{ManageWheelCasesUseCase, WheelCases};
use nexus_core::ports::outbound::casino::game_repository::{Game, GamePanel, GameRepository};
use nexus_core::ports::outbound::coussin_insurance_repository::CoussinInsurance as OutboundInsurance;
use nexus_core::ports::outbound::coussin_inventory_repository::InventoryItem;
use nexus_core::ports::outbound::coussin_repository::{
...
use nexus_core::ports::outbound::game::container_runtime::{ContainerStats, MockContainerRuntime};
...
use nexus_proto::game::v1::game_server_service_server::GameServerService;
use nexus_proto::game::v1::{ExecuteRconRequest, StreamLogsRequest, StreamStatsRequest};
...
// ── Mocks conformes à 100% avec les traits de nexus-core ──
...
impl CoussinStealUseCase for DummyCoussinSteal {
    async fn steal(
        ...
        _: &str,
        _: bool,
    ) -> Result<nexus_core::ports::inbound::coussin_steal::StealResult, DomainError> {
        todo!()
    }
}
...
impl GameServerRepository for DummyGameServerRepo {
    ...
    async fn update_runtime(
        &self,
        _: Uuid,
        _: nexus_core::ports::outbound::game::game_server_repository::GameServerRuntimeUpdate,
    ) -> Result<(), DomainError> {
        Ok(())
    }
...
impl GameAuditRepository for DummyAuditRepo {
    async fn log(
        ...
        _: Option<Uuid>,
        _: Option<&str>,
        _: nexus_core::domain::entities::game::audit::GameAuditAction,
        _: serde_json::Value,
    ) -> Result<(), DomainError> {
        Ok(())
    }
    async fn list_for_server(
        ...
        _: Uuid,
        _: i64,
    ) -> Result<Vec<nexus_core::domain::entities::game::audit::GameAuditEntry>, DomainError> {
        Ok(vec![])
    }
...
impl PlayerSessionRepository for DummyPlayerSessionRepo {
    ...
    async fn list_active(
        &self,
        _: Uuid,
    ) -> Result<Vec<nexus_core::domain::entities::game::player_session::PlayerSession>, DomainError>
    {
        Ok(vec![])
    }
...
impl RconClient for DummyRconClient {
    async fn execute(
        &self,
        _: &nexus_core::ports::outbound::game::rcon_client::RconConnectionParams,
        _: &str,
    ) -> Result<nexus_core::ports::outbound::game::rcon_client::RconResponse, DomainError> {
        todo!()
    }
}
