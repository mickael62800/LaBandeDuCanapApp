-- 073_zomboid_bac_a_sable_complet.sql
--
-- Porte le bac a sable de Zomboid a la totalite du jeu de base : cent trente
-- reglages, contre vingt-six poses par la migration 072.
--
-- POURQUOI UNE SECONDE MIGRATION plutot qu'une 072 corrigee. La 072 etait deja
-- appliquee. Modifier un fichier de migration passe fait echouer le demarrage
-- de l'API — sqlx en garde une empreinte et refuse de la voir changer. La
-- corriger sur place aurait donc casse l'installation au lieu de l'ameliorer,
-- et c'est exactement ce qui s'est produit avant que cette migration n'existe.
--
-- ELLE RETIRE PUIS REPOSE. Les vingt-six cles de la 072 sont supprimees avant
-- que les cent trente ne soient ajoutees : les ajouter par-dessus aurait laisse
-- vingt-six doublons, deux champs pour un meme reglage, dont le dernier rendu
-- aurait gagne en silence. C'est le defaut que la migration 064 avait deja du
-- reparer sur Minecraft.
--
-- TOUS LES CHAMPS SONT VIDES PAR DEFAUT. La 072 posait des defauts « normaux »
-- (Zombies = 3, butin = 4) ; ils paraissent inoffensifs mais ECRIVENT le
-- reglage, donc figent la partie sur ces valeurs meme si l'exploitant n'a rien
-- touche. Vide, aucune ligne n'est ecrite et le jeu applique SES defauts.
--
-- La liste est generee depuis la meme source que la table Rust qui compose le
-- fichier `SandboxVars.lua` : cent trente noms recopies deux fois auraient
-- produit des ecarts qu'aucun message d'erreur n'aurait signales, une cle
-- inconnue du jeu etant simplement ignoree.

-- 1. Retrait des cles de bac a sable existantes.
--
-- `SANDBOX\_%` : le souligne est un joker dans LIKE, il faut l'echapper. Sans
-- cela, le motif emporterait aussi des cles nommees « SANDBOXx... ».
UPDATE game_templates
SET config_schema = COALESCE(
    (
        SELECT jsonb_agg(champ ORDER BY ord)
        FROM jsonb_array_elements(config_schema) WITH ORDINALITY AS t(champ, ord)
        WHERE champ ->> 'key' IS NULL
           OR champ ->> 'key' NOT LIKE 'SANDBOX\_%'
    ),
    '[]'::jsonb
)
WHERE slug = 'project-zomboid';

-- 2. Pose des cent trente reglages du jeu de base.
