//! Quotas persistants appliqués avant chaque appel payant au fournisseur IA.
//!
//! Le contrôle est transactionnel : les compteurs membre et guilde sont
//! verrouillés avant l'incrément, ce qui empêche deux requêtes concurrentes de
//! dépasser une limite.
//!
...
use crate::atrium::{
    guild_config::{self, ConfigDefaults, GuildSettings},
    AppConfig,
};
...
pub struct BudgetStats {
    /// Requetes consommees aujourd'hui par la guilde demandee, comptees sur le
    /// compteur « plateforme » (celui que borne `global_daily_limit`).
...
impl BudgetGuard {
    ...
    /// Reglages effectifs a partir d'un instantane de config deja charge.
    ///
    /// P2 : la config du serveur est lue UNE fois par requete, en amont ; les
    /// limites en sont deduites ici sans repasser par la base.
    pub fn settings_from_map(&self, raw: &HashMap<String, String>) -> GuildSettings {
        guild_config::from_map(raw, self.defaults)
    }
    ...
    ///
    /// Les `limits` sont fournies par l'appelant (instantane de config resolu en
    /// debut de requete) plutot que rechargees ici : c'etait la 2e des trois
    /// lectures de `bot_guild_config` d'une meme requete d'accueil.
    pub async fn check_and_record(
        &self,
        guild_id: &str,
        user_id: &str,
        interactive: bool,
        limits: &GuildSettings,
    ) -> Result<Option<String>, sqlx::Error> {
        let mut tx = self.pool.begin().await?;

        // La retention (purge des vieilles lignes) n'est PLUS faite ici : elle
        // rallongeait la transaction qui verrouille les compteurs, a chaque
        // appel IA, alors qu'une purge quotidienne suffit. Elle est desormais
        // declenchee par atrium-worker via `purge_old`.
        sqlx::query(
        ...
        sqlx::query(
            "INSERT INTO atrium_ai_usage_global (usage_date, guild_id) VALUES (CURRENT_DATE, $1) \
        ...
        let global_count: i32 = sqlx::query(
            "SELECT request_count FROM atrium_ai_usage_global \
             WHERE usage_date = CURRENT_DATE AND guild_id = $1 FOR UPDATE",
        )
        .bind(guild_id)
        .fetch_one(&mut *tx)
        .await?
    ...
    /// Purge quotidienne des compteurs de quota trop anciens.
    ///
    /// Sortie du chemin critique de `check_and_record` : conservee hors de la
    /// transaction qui verrouille les compteurs, elle n'est declenchee qu'une
    /// fois par jour par atrium-worker. On garde une courte fenetre (7 jours)
    /// suffisante au diagnostic sans faire grossir les tables indefiniment.
    pub async fn purge_old(&self) -> Result<(), sqlx::Error> {
