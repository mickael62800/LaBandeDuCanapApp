...
impl GameServerRepository for PgGameServerRepository {
    ...
    async fn compter_tentative_annonce(&self, id: Uuid) -> Result<(), DomainError> {
        sqlx::query(
            "UPDATE game_servers SET announcement_attempts = announcement_attempts + 1, \
             updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL",
        )
        .bind(id)
        .execute(&self.pool)
        .await
        .map_err(pg_ctx("compter_tentative_annonce"))?;
        Ok(())
    }

    async fn marquer_annonce_publiee(&self, id: Uuid) -> Result<(), DomainError> {
        sqlx::query(
            "UPDATE game_servers SET announcement_posted_at = NOW(), updated_at = NOW() \
             WHERE id = $1 AND announcement_posted_at IS NULL",
        )
        .bind(id)
        .execute(&self.pool)
        .await
        .map_err(pg_ctx("marquer_annonce_publiee"))?;
        Ok(())
    }
