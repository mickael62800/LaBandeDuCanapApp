//! Configuration lue au démarrage de l'API Ops.
//!
//! Les secrets d'administration sont obligatoires. Le token métriques peut
//! rester vide uniquement lorsque l'exposition est limitée au réseau interne.
...
pub struct AppConfig {
    ...
    pub security_alerts_webhook: String,
}
...
impl AppConfig {
    pub fn from_env() -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            rate_limit_per_sec: std::env::var("OPS_API_RATE_LIMIT_PER_SEC")
                ...
                .unwrap_or(20),
            security_alerts_webhook: std::env::var("SECURITY_ALERTS_WEBHOOK").unwrap_or_default(),
        })
    }
    ...
    pub fn dummy() -> Self {
        Self {
            ...
            security_alerts_webhook: String::new(),
        }
    }
}
