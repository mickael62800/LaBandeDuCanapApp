...
pub struct ContainerChangesResponse {
    ...
    pub current: Vec<platform_core::ops::domain::entities::container_monitor::ContainerSnapshot>,
    pub changes_24h:
        Vec<platform_core::ops::domain::entities::container_monitor::ContainerChangeEntry>,
}

/// Lit l'etat publie dans Redis par `ops-agent`.
pub async fn container_changes(
    State(state): State<AppState>,
) -> Result<Json<ContainerChangesResponse>, ApiError> {
    let snapshot = crate::ops::container_monitor::load(&state.redis_client)
        .await
        .map_err(|error| {
