...
use platform_proto::sentinel::moderation::v1 as proto;
use platform_proto::sentinel::moderation::v1::moderation_service_server::ModerationService;
use tonic::Request;
use tonic::Response;
use tonic::Status;
...
use platform_core::sentinel::ports::inbound::moderation::assess_target_risk::{
    AssessTargetRiskCommand, AssessTargetRiskUseCase,
};
use platform_core::sentinel::ports::inbound::moderation::cancel_action::{
    CancelModerationActionUseCase, CancelOutcome,
};
use platform_core::sentinel::ports::inbound::moderation::manage_infractions::ManageInfractionsUseCase;
...
use platform_core::sentinel::ports::inbound::moderation::read_modstats::ReadModstatsUseCase;
...
pub struct ModerationGrpc {
    ...
    pub manage_reminders_uc: Arc<
        dyn platform_core::sentinel::ports::inbound::moderation::manage_reminders::ManageRemindersUseCase,
    >,
}
    ...
    async fn log_action(
        ...
        if let Some(duration_secs) = req.duration {
            let _ = self
                .manage_reminders_uc
                .create_reminder(
                    platform_core::sentinel::ports::inbound::moderation::manage_reminders::CreateReminderCommand {
...
fn evidence_to_proto(
    e: platform_core::sentinel::ports::outbound::moderation::evidence_repository::EvidenceEntry,
) -> proto::EvidenceEntry {
    proto::EvidenceEntry {
        id: e.id.to_string(),
...
fn review_to_proto(
    r: platform_core::sentinel::ports::outbound::moderation::review_repository::ReviewEntry,
) -> proto::ReviewEntry {
    proto::ReviewEntry {
        id: r.id.to_string(),
