...
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::CreateLogDto;
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::DashboardInfractionDto;
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::DashboardRuleDto;
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::DashboardStatsDto;
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::GuildDto;
use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::GuildFilterParams;

use crate::sentinel::adapters::inbound::http::dto::audit::dashboard::RegisterGuildDto;
use tracing::warn;

use crate::sentinel::adapters::inbound::http::errors::ApiError;
use crate::sentinel::adapters::outbound::system::redis_log_stream;
use crate::sentinel::bootstrap::state::DashboardState;
...
pub async fn get_dashboard_stats(
    ...
    let counts = state.service_registry.count_services().await;
    let health = platform_core::ops::domain::entities::services_health::ServicesHealth {
        ...
        redis_online: state.service_registry.ping().await,
    };
...
pub async fn create_log(
    ...
    let category = dto.category.unwrap_or_else(|| {
        platform_core::sentinel::domain::entities::system::config_parsers::default_log_category(
            &bot_name,
        )
        .to_string()
    });
...
/// - Evenements `mod_*` de `audit_logs` : sanctions prises (warn/mute/ban/unban) avec
...
pub async fn get_all_infractions(
    ...
    let infractions = match &params.guild_id {
        Some(gid) => {
            let filters =
                platform_core::sentinel::ports::inbound::moderation::manage_infractions::InfractionFilters {
                    user_id: None,
                    action: None,
                    limit: 200,
                    offset: 0,
...
pub async fn register_guild(
    ...
    let guild = platform_core::sentinel::domain::entities::system::guild::Guild {
...
async fn invalidate_guilds_cache(state: &DashboardState) {
    if let Ok(mut conn) = state.redis_client.get_multiplexed_async_connection().await {
        if let Err(e) = conn.del::<_, ()>("guilds:all").await {
            warn!(error = %e, "Echec invalidation cache guilds:all");
        }
