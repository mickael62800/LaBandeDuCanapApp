...
pub async fn add_evidence(
    ...
    platform_core::sentinel::domain::entities::moderation::review::manual::validate_evidence_url(
        &dto.url,
    )
    .map_err(|m| {
        ApiError(platform_core::sentinel::domain::errors::DomainError::ValidationError(m.into()))
    })?;
    ...
    let description = dto.description.as_deref().map(
