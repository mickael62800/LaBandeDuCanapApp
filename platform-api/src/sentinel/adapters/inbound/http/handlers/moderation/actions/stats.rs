...
pub async fn get_modstats(
    ...
    Json<Vec<crate::sentinel::adapters::inbound::http::dto::moderation::actions::ModStatsEntryDto>>,
    ApiError,
> {
    let days =
        (crate::sentinel::adapters::inbound::http::helpers::normalize_in(params.days, 30, 1, 90))
            as i32;
    ...
    let dtos = rows
        .into_iter()
        .map(|r| {
            crate::sentinel::adapters::inbound::http::dto::moderation::actions::ModStatsEntryDto {
...
pub async fn delete_action(
    ...
    use platform_core::sentinel::ports::inbound::moderation::cancel_action::CancelOutcome;
...
pub async fn mod_action_count(
    ...
    let window =
        platform_core::sentinel::domain::entities::moderation::action::reversal::mod_action_window_secs(
            q.window_secs,
        );
    let count = state
        .moderation_uc
