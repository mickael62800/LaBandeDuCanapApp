...
pub async fn run(pool: &PgPool, redis: &redis::aio::ConnectionManager) -> Result<(), String> {
    ...
    for ticket in &candidates {
        ...
        if !crate::sentinel::jobs::support::is_enabled(pool, &ticket.server, "ticket-bot").await {
            skipped += 1;
            continue;
        }
        let escalation_minutes =
            platform_core::sentinel::domain::services::tickets::sla::effective_threshold(
                sla_configs
                    .get(&ticket.server)
        ...
        if !platform_core::sentinel::domain::services::tickets::sla::is_breached(
            age_minutes,
            escalation_minutes,
        ) {
            skipped += 1;
