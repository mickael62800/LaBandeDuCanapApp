...
//!
//! # Pourquoi ce connecteur reste local à Sentinel
//!
//! Ce module s'appuie sur `platform_proto::sentinel::tls` (certificats mTLS de la
...
pub async fn connect() -> Result<Channel, String> {
    ...
    let effective_url = if platform_proto::sentinel::tls::tls_dir().is_some() {
    ...
    let endpoint = match platform_proto::sentinel::tls::tls_dir() {
        Some(dir) => {
            let domain = url
                .strip_prefix("http://")
                ...
                .next()
                .unwrap_or("api");
            let tls = platform_proto::sentinel::tls::client_tls_config(&dir, domain)
