...
use platform_core::sentinel::domain::entities::system::config_parsers::parse_bool_str;

use chrono::{DateTime, Utc};
use sqlx::PgPool;
use tracing::{debug, info, warn};

use crate::sentinel::jobs::support;
...
pub async fn run(pool: &PgPool, redis: &redis::aio::ConnectionManager) -> Result<(), String> {
    ...
    for (guild_id, cfg) in &configs {
        ...
        match support::publish_event(&mut conn, &payload.to_string()).await {
            Ok(_) => published += 1,
            Err(e) => {
...
fn is_due(last: Option<DateTime<Utc>>, interval_hours: i64, now: DateTime<Utc>) -> bool {
    platform_core::sentinel::domain::services::system::scheduling::is_due(
        last,
        interval_hours,
