...
fn handlers_do_not_depend_on_app_state_or_execute_sql() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/adapters/inbound/http/handlers");
...
fn app_state_is_only_an_aggregate_of_substates() {
    ...
    let source = std::fs::read_to_string(root.join("src/bootstrap/state.rs"))
...
fn audit_log_migrations_cover_the_verified_access_paths() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let query_indexes =
        std::fs::read_to_string(root.join("migrations/032_audit_logs_query_indexes.sql"))
            .expect("lecture migration index audit_logs");
    let discord_index =
        std::fs::read_to_string(root.join("migrations/031_discord_audit_sync_idempotency.sql"))
