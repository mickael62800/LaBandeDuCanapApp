...
async function load(){const g=selectedGuildId.value,u=user.value;if(!g||!u)return;loading.value=true;error.value="";try{const [p,m,a,c]=await Promise.all([api.membership(g,u.id),api.motions(g),api.gazette(g),api.cercles(g)]);const d=p?await api.dossiers(g,u.id):[];profile.value=p;motions.value=m;articles.value=a;cercles.value=c;dossiers.value=d}catch(e){error.value=e instanceof Error?e.message:"Chargement impossible"}finally{loading.value=false}}
